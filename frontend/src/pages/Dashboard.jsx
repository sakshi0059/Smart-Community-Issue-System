...
const Dashboard = () => {
  ...
  const fetchUserIssues = async () => {
    ...
    try {
      ...
      // FIX: Always set an array so .filter/.map never crash
      setUserIssues(Array.isArray(issues) ? issues : []);

    } catch (error) {
      console.error("Error fetching user issues:", error);
      setUserIssues([]); // fail-safe
    } finally {
      setLoading(false);
    }
  };
                ...
                <ul className="mb-6">
                  {userIssues.slice(0, 5).map(issue => (
                    <li
                      ...
                      key={issue._id}
                      className="bg-white cursor-pointer rounded-lg shadow p-4 mb-2 flex justify-between items-center"
                    >
                      <span className="font-medium text-zinc-700">{issue.title}</span>

                      <span
                        className={`px-3 py-1 capitalize rounded-full text-xs font-bold ${
